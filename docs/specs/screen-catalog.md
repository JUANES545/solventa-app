...
| AUTH-02 | Sign in | Submit demo credentials, enter as test user, biometric entry, recover password, open registration | Incomplete fields, invalid credentials, submitting, success, login error |
...
| DBG-01 | Test scenarios | Select normal, empty lists, network error, slow response, expired session, successful claim, failed claim | Debug builds only; current scenario visible |
